...
Desde `web/` (los PNG se guardan aquí, en `pitch/qr/`):

```bash
pnpm qr:generate
```

Usa la lista en `web/src/data/qr-lots.ts`. La URL base es `NEXT_PUBLIC_APP_URL` o, si es localhost, la URL de producción del hackathon.
...
2. Añade la entrada en `web/src/data/qr-lots.ts`.
3. `pnpm qr:generate` (desde `web/`).
